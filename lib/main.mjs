...
(async function DaedalusApplication() {
  ...
  const clearState = async () => {
    ...
    mongoConnection = await mongodb.init({ resetUser: true });
  ...
  let mongoConnection = await mongodb.init();

  const POSTPersona = await (async () => {
    return async function(persona, force) {
      if (!force && (!persona || state.lastPersonaSent == persona)) return;
      console.log(`Posting persona "${persona}"`);

      const userObj = await mongodb.postPersona(mongoConnection, persona);
      state.lastPersonaSent = persona;

      state.leaf.metaData = leaf.setMetaData({
        mongodbClient: mongoConnection.client,
        userObj
      });
    };
  })();
