...
function renderUIContent({ appConfig, appState }) {
  ...
  const scrollBursts = appState.leaf.pages.reduce((sum, page) => {
    appConfig.CBD.scrollBurstThresholds.forEach((nbScrolls, i) => {
      sum[i] =
        (sum[i] || 0) + page.mouse.scrollBursts[i] * (nbScrolls * 1 || 1);
    });
    return sum;
  }, []);
  const nbScrolls = appState.leaf.pages.reduce(
    (sum, page) =>
        ...
        <div>
          ...
          <div class="cbd-list">
            <span class=${"cbd-value" +
              (appState.CBD.sentiment == "positive"
                ? " active"
                : "")}><img class="avatar" src="asset/emoji-happy.svg" /></span>
            <span class=${"cbd-value" +
              (!appState.CBD.sentiment || appState.CBD.sentiment == "neutral"
                ? " active"
                : "")}><img class="avatar" src="asset/emoji-neutral.svg" /></span>
            <span class=${"cbd-value" +
              (appState.CBD.sentiment == "negative"
                ? " active"
